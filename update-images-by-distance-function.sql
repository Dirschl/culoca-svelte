-- Update images_by_distance function to use higher default page size
-- This ensures more images are loaded per request

-- Drop the existing function
DROP FUNCTION IF EXISTS images_by_distance;

-- Create the updated function with higher default page size
CREATE OR REPLACE FUNCTION images_by_distance(
  user_lat DOUBLE PRECISION,
  user_lon DOUBLE PRECISION,
  page INTEGER DEFAULT 0,
  page_size INTEGER DEFAULT 1000  -- Increased from 500 to 1000
)
RETURNS TABLE(
  id UUID,
  path_512 TEXT,
...
-- Test the function
SELECT 'Function updated successfully with higher default page size' as status; 