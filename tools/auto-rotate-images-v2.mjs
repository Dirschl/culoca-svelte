 ...
 * Uses WebDAV to download originals from Hetzner
 */
  ...
  try {
    ...
    if (targetItemId) {
      ...
      const { data, error } = await supabase
        .from('items')
        .select('id, path_512, path_2048, path_64, original_name, original_url')
        .eq('id', targetItemId);
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (item.original_url) {
          ...
          originalPath = `items/${item.id}.jpg`;
        }
        
        console.log(`📁 Original path: ${originalPath}`);
        
        // Download original from Hetzner via WebDAV
        let originalBuffer;
        try {
          originalBuffer = await webdav.getFileContents(originalPath, { format: 'binary' });
