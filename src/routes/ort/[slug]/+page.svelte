<script lang="ts">
  ...
  const pageTitle =
    data.page > 1
      ...
      : `${data.hubLabel}: Ort und Motive | Culoca`;
  const metaDescription = trimText(
    `Entdecke ${data.totalCount} öffentliche Inhalte mit Ortsbezug zu ${data.hubLabel} auf Culoca. Semantische Übersichtsseite für Motive, Themen und Detailseiten.`
  );
...
<svelte:head>
  ...
  {@html `<script type="application/ld+json">${JSON.stringify(buildBreadcrumbJsonLd([
    { name: 'Culoca', path: '/' },
    { name: 'Orte', path: PRIMARY_REGIONAL_FEED_PATH },
  ...
  <main class="hub-page">
    <div class="hub-inner">
      <nav class="breadcrumb" aria-label="Breadcrumb">
        <a href="/">Culoca</a>
        <span>/</span>
        <a href={PRIMARY_REGIONAL_FEED_PATH}>Region</a>
        <span>/</span>
        <span>{data.hubLabel}</span>
      </nav>
      <header class="hub-header">
        <p class="hub-kicker">Orts-Hub</p>
        <h1>{data.hubLabel}</h1>
        <p class="hub-copy">
          Diese Ortsseite stärkt interne Verlinkung und fasst Inhalte zusammen, die im Event-Kontext oder in Keywords
          explizit auf <strong>{data.hubLabel}</strong> verweisen.
        </p>
      </header>
        ...
        {#each data.items as item (item.id)}
          <article class="item-card">
            <a href={itemHref(item)}>
              ...
              <p>{trimText(item.description || item.caption || `Mehr Inhalte aus ${data.hubLabel}.`, 110)}</p>
            </a>
          </article>
        {/each}
      </div>
