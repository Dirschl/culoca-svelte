...
export const load: PageServerLoad = async ({ params, url }) => {
  ...
  if (!profile?.id) {
    throw error(404, 'Fotograf nicht gefunden');
  }
  ...
  const { data: items, error: itemsError } = await supabase
    .from('items')
    .select('id, slug, title, description, caption, canonical_path, country_slug, district_slug, municipality_slug, path_512, width, height, created_at')
    .eq('profile_id', profile.id)
    .eq('admin_hidden', false)
    .is('group_root_item_id', null)
    .not('slug', 'is', null)
  ...
  return {
    ...
    page,
    totalPages,
    hubPath: `/fotograf/${accountname}`,
    seoPolicy: getHubSeoPolicy({
      basePath: `/fotograf/${accountname}`,
      page
    })
  };
};
