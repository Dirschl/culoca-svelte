<script lang="ts">
	...
	let availableTypes: Array<{ id: number; name?: string; slug?: string }> =
		data?.availableTypes ?? [];
	...
	let attribution = data?.attribution ?? null;
	let seoHubs = data?.seoHubs ?? {
		typePath: null,
		typeLabel: null,
		keywordLinks: [],
		photographerPath: null,
	...
	$: if (data?.image?.id && data.image.id !== image?.id) {
		...
		canonicalPath =
			data?.canonicalPath ?? (data.image?.slug ? `/item/${data.image.slug}` : canonicalPath);
	...
	$: nearbyFallbackRecommendations = [
		...(seoHubs?.placePath && seoHubs?.placeLabel
			? [
					{
						title: `Mehr Inhalte aus ${seoHubs.placeLabel}`,
						href: seoHubs.placePath,
						description: 'Ortsbezogener Hub mit weiteren Details und Motiven.'
					}
				]
			: []),
		...(seoHubs?.photographerPath && seoHubs?.photographerLabel
			? [
					{
						title: `Mehr von ${seoHubs.photographerLabel}`,
						href: seoHubs.photographerPath,
						description: 'Alle öffentlichen Inhalte dieses Fotografen.'
					}
				]
			: []),
		...(seoHubs?.typePath && seoHubs?.typeLabel
			? [
					{
						title: `${seoHubs.typeLabel} entdecken`,
						href: seoHubs.typePath,
						description: 'Zur kuratierten Übersicht dieses Seitentyps.'
					}
				]
			: [])
	];
	...
	$: geoCountryLabel =
		normalizeAdminDisplayLabel(
			geoAdminHierarchy.countryName ||
				image?.country_name ||
				image?.country_slug?.toUpperCase() ||
				image?.country_code
		) || 'Land';
	$: geoStateLabel = normalizeAdminDisplayLabel(
		image?.state_name || geoAdminHierarchy.stateName || null
	);
	$: geoRegionLabel = normalizeAdminDisplayLabel(
	...
	function buildMainImageAlt(
		img:
			| { title?: string | null; caption?: string | null; description?: string | null }
			| null
			| undefined
	): string {
		if (!img) return '';
	...
	$: metaAuthor = attribution?.authorMeta ?? attribution?.creatorName ?? '';
	$: metaCopyright = attribution?.copyrightNotice ?? '';
	...
	$: hasDateRange = !!(
		contentType?.show_date_range &&
		(contextItem?.starts_at || contextItem?.ends_at)
	);
	...
	$: hasEventDetails =
		...
		!!(
			eventScheduleText ||
			eventSettings.location_name ||
			eventSettings.booking_url ||
			eventSettings.is_free ||
			eventSettings.price_text
		);
	...
	$: currentVariantRootId = image?.group_root_item_id
		? rootItem?.id || image.group_root_item_id
		: image?.id || null;
	...
	function dedupeGeoCrumbs<T extends { name: string; path: string | null }>(crumbs: T[]): T[] {
		...
		for (const crumb of crumbs) {
			...
			if (
				last &&
				normalizeGeoComparisonValue(last.name) === normalizeGeoComparisonValue(crumb.name)
			) {
	...
	function buildPageSettings(
		...
		const nextSettings = buildEventPageSettings(
			currentSettings,
			nextEventSettings,
	...
	$: calendarUrl = image?.id ? `/api/events/${image.id}/ics` : null;
	...
	function getLocalReferrerFallback(): string {
		...
		try {
			...
			if (referrerUrl.pathname === $page.url.pathname && referrerUrl.search === $page.url.search)
				return '/';
	...
	async function loadItemChatUnreadCount() {
		...
		itemChatUnreadCount = data.filter((entry: any) => {
			...
			return (
				!ownReadAt || new Date(entry.last_message_at).getTime() > new Date(ownReadAt).getTime()
			);
		}).length;
	}
		...
		itemChatChannel = supabase
			...
			.on(
				'postgres_changes',
				{
					...
					filter: `user_a_id=eq.${itemChatUserId}`
				},
			...
			.on(
				'postgres_changes',
				{
					...
					schema: 'public',
					table: 'user_conversations',
					filter: `user_b_id=eq.${itemChatUserId}`
				},
				async () => loadItemChatUnreadCount()
			)
			.subscribe();
	...
	async function loadComments() {
        ...
        `
			...
			comments = isCreator
				? data || []
				: (data || []).filter((entry: any) => entry.status === 'visible');
		} catch (error) {
	...
	function getInteractionActorLabel(entry: any) {
		return (
			...
			entry?.profiles?.accountname ||
			(entry?.actor_user_id ? 'Nutzer' : 'Anonym')
		);
	}
	...
	async function toggleCommentVisibility(comment: any) {
		...
		try {
			...
			commentStatus =
				nextStatus === 'hidden' ? 'Kommentar ausgeblendet.' : 'Kommentar wieder sichtbar.';
	...
	async function toggleFavorite() {
		...
		try {
			if (isFavorited) {
				...
				const { error } = await supabase.from('item_favorites').upsert(
					{
						user_id: currentUser.id,
						item_id: image.id,
	...
	function setUserFilter() {
		...
		const username =
			image.profile?.full_name || image.profile?.accountname || image.profile_id || 'Profil';
		const accountName =
			image.profile?.accountname || image.profile?.full_name || image.profile_id || 'Profil';
	...
	function syncManagementFormFromImage() {
		...
		managementForm = {
			...
			sort_order:
				image.sort_order === null || image.sort_order === undefined ? '' : String(image.sort_order),
	...
	async function persistGeoFields(
		countryName: string,
		districtName: string,
		...
		try {
			...
			if (browser) {
				const targetPath =
					getStoredOrComputedCanonicalPath({
						item: image,
						rootItem: image?.group_root_item_id
	...
	async function saveGeoFields() {
		if (
			...
			!currentUser ||
			!image ||
			!(isCreator || $unifiedRightsStore.rights?.edit)
		)
			return;
		await persistGeoFields(
			countryNameEditValue,
			districtNameEditValue,
			municipalityNameEditValue,
			localityNameEditValue
		);
		editingGeoFields = false;
	}
	...
	async function autofillGeoFieldsFromCoordinates() {
		...
		try {
			...
			editingGeoFields = true;
			countryNameEditValue =
				geocoded.countryName || countryNameEditValue || image.country_name || '';
			districtNameEditValue =
				geocoded.districtName || districtNameEditValue || image.district_name || '';
			municipalityNameEditValue =
				geocoded.municipalityName || municipalityNameEditValue || image.municipality_name || '';
			localityNameEditValue =
				geocoded.localityName || localityNameEditValue || image.locality_name || '';
			await persistGeoFields(
				countryNameEditValue,
				districtNameEditValue,
				municipalityNameEditValue,
	...
	async function loadRootCandidates(query: string) {
		...
		try {
			...
			if (sanitized) {
				request = request.or(
					`title.ilike.%${sanitized}%,slug.ilike.%${sanitized}%,group_slug.ilike.%${sanitized}%`
				);
			}
	...
	async function saveManagementFields(options: { navigate?: boolean } = {}) {
		...
		const payload = {
			...
			sort_order:
				managementForm.sort_order.trim() === '' ? null : Number(managementForm.sort_order),
	...
	async function saveTitle() {
		if (!editingTitle || !currentUser || !image || !(isCreator || $unifiedRightsStore.rights?.edit))
			return;
	...
	async function saveSlug() {
		if (!editingSlug || !currentUser || !image || !(isCreator || $unifiedRightsStore.rights?.edit))
			return;
	...
	async function handleNearbyGalleryToggle(itemId: string, nextAssignedState: boolean) {
		...
		try {
			...
			if (browser) {
				window.alert(
					error instanceof Error ? error.message : 'Variante konnte nicht gespeichert werden'
				);
			}
		}
	}
	...
	let fileSizes: { size64: number | null; size512: number | null; size2048: number | null } = {
		size64: null,
		size512: null,
		size2048: null
	};
	...
	async function fetchFileSizes() {
		...
		const [size64, size512, size2048] = await Promise.all([
			...
			fetchRemoteFileSize(
				buildSeoSizedImageUrl('512') || buildPublicStorageUrl('images-512', image.path_512)
			),
			fetchRemoteFileSize(
				buildSeoSizedImageUrl('2048') || buildPublicStorageUrl('images-2048', image.path_2048)
			)
		]);
	...
	async function saveAdobeStockData() {
		...
		try {
			...
			const nextStatus = normalizedUrl
				? image.adobe_stock_status === 'none'
					? 'uploaded'
					: image.adobe_stock_status
				: 'none';
...
<svelte:head>
	...
	{#if image}
		...
		{@const extensionMatch = imagePathForExtension
			? imagePathForExtension.match(/\.(jpg|jpeg|webp|png)$/i)
			: null}
		{@const fileExtension = extensionMatch ? extensionMatch[0].toLowerCase() : '.jpg'}
		...
		{@const scale2048 =
			originalWidth > maxDimension2048 || originalHeight > maxDimension2048
				? Math.min(maxDimension2048 / originalWidth, maxDimension2048 / originalHeight)
				: 1}
		{@const width2048 = Math.max(
			1,
			Math.min(Math.round(originalWidth * scale2048), maxDimension2048)
		)}
		{@const height2048 = Math.max(
			1,
			Math.min(Math.round(originalHeight * scale2048), maxDimension2048)
		)}
		{#if ogImageUrl}
	...
	<link
		rel="icon"
		type="image/png"
		href={`/api/favicon/${itemSlug}`}
		sizes="32x32 48x48 96x96 192x192 512x512"
	/>
	...
	{#if image}
		...
		{@const dateModified = image.updated_at
			? new Date(image.updated_at).toISOString()
			: image.created_at
		...
		{@const rawExifCaption =
			image.exif_data?.Caption != null ? String(image.exif_data.Caption) : ''}
		...
		{@const creditText = attribution?.creditText ?? ''}
		{@const copyrightNotice = attribution?.copyrightNotice ?? ''}
		...
		{@const normalizedSchemaCreatorName = normalizeUtf8(attribution?.creatorName ?? '')}
		{@const normalizedSchemaCopyrightOrgName = normalizeUtf8(
			attribution?.copyrightHolderName ?? ''
		)}
		...
		{:else if image}
							...
							<div class="title-event-details-card">
								...
								<div class="title-event-details-grid">
										...
										<div class="datetime-input-wrap">
											...
											<span class="datetime-input-icon" aria-hidden="true">
												<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
													<path
														d="M7 2h2v2h6V2h2v2h2a2 2 0 0 1 2 2v13a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3V6a2 2 0 0 1 2-2h2V2zm12 8H5v9a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-9zM6 6a1 1 0 0 0-1 1v1h14V7a1 1 0 0 0-1-1H6zm2 6h3v3H8v-3z"
													/>
												</svg>
											</span>
										</div>
									...
									<label class="title-event-field title-event-field-full">
										...
										<input
											type="text"
											bind:value={managementForm.event_location_name}
											placeholder="z.B. Stadthalle Burghausen"
										/>
									</label>
									...
									<label class="title-event-field title-event-field-full">
										...
										<input
											type="url"
											bind:value={managementForm.event_booking_url}
											placeholder="https://..."
											autocomplete="off"
											autocorrect="off"
											autocapitalize="none"
									...
									<label class="title-event-field">
										...
										<input
											type="text"
											bind:value={managementForm.event_price_text}
											placeholder="z.B. 12 EUR"
										/>
									</label>
								</div>
							</div>
						...
						<div class="title-group-slug-field">
							...
							{#if showGroupSlugInfo}
								<div class="group-slug-info-box">
									Vergeben sie einen eindeutigen Gruppen Slug, falls Unterelemente hinzugefügt
									werden sollen. Sie erhalten dann eine eigene Landingpage für z.B. Musikalben,
									Bildergalerien etc...
								</div>
							{/if}
							...
							{:else if groupSlugSuggestions.length > 0}
								<div class="group-slug-suggestions">
									{#each groupSlugSuggestions as slug}
										<button
											type="button"
											class="group-slug-suggestion"
											on:click={() => applyGroupSlugSuggestion(slug)}
										>
											{slug}
										</button>
									{/each}
							...
							{:else}
								<div class="group-slug-hint">
									Leer lassen, wenn kein sichtbarer Gruppenpfad gebraucht wird.
								</div>
							{/if}
						</div>
					{/if}
								...
								<button type="button" class="caption-text" on:click={startEditCaption}>
									...
									{:else}
										<em class="placeholder"
											>Klicke hier um eine emotionale Beschreibung hinzuzufügen</em
										>
									{/if}
								</button>
							{/if}
							...
							<div class="event-detail-list">
								...
								{#if eventSettings.booking_url}
									<a
										...
										href={eventSettings.booking_url}
										target="_blank"
			...
			{#if image?.adobe_stock_url && !(canEditItem && editMode)}
				<p class="adobe-stock-cta">
					<a
						href={getAdobeAffiliateUrl(image.adobe_stock_url) || image.adobe_stock_url}
						target="_blank"
						rel="noopener noreferrer sponsored">Bei Adobe Stock ansehen</a
					>
				</p>
			{/if}
								...
								<div class="comment-actions">
									...
									{#if isCreator}
										<button
											type="button"
											class="comment-delete-btn"
											on:click={() => toggleCommentVisibility(comment)}
										>
									...
									<div class="comment-form-footer">
										...
										<div class="comment-inline-actions">
											<button
												type="button"
												class="comment-delete-btn"
												on:click={() => (activeReplyCommentId = null)}
											>
												Abbrechen
											</button>
											<button
								...
								<div class="comment-replies">
									{#each getReplies(comment.id) as reply}
										<article
											...
											class:is-hidden={reply.status === 'hidden'}
										>
											<div class="comment-avatar">
											...
											<div class="comment-actions">
												{#if isCreator}
													<button
														type="button"
														class="comment-delete-btn"
														on:click={() => toggleCommentVisibility(reply)}
													>
			...
			{#if shouldShowNearbyGallery}
				<div class="radius-control">
					...
					<input
						...
						step="50"
						value={radius}
						on:input={onRadiusInput}
						on:change={onRadiusChange}
						class:limit-reached={isAtItemLimit}
					/>
				</div>
			{/if}
			{#if shouldShowNearbyGallery}
			...
			<div class="meta-section single-exif">
				...
				<div class="keywords-column">
					<button
						...
						class:editable={canEditItem && editMode}
						class:editing={editingKeywords}
						on:click={startEditKeywords}
					>
						Keywords
					</button>
					{#if editingKeywords}
						<div class="keywords-edit-container">
							...
							<span
								class="char-count"
								class:limit-reached={keywordsEditValue.split(',').filter((k) => k.trim().length > 0)
									.length >= KEYWORDS_MAX}
								>{keywordsEditValue.split(',').filter((k) => k.trim().length > 0)
									.length}/{KEYWORDS_MAX}</span
							>
						</div>
					{:else if image.keywords && image.keywords.length}
						<div class="keywords">
					...
					{:else}
						<div class="keywords-placeholder">
							{isCreator
								? 'Klicke auf "Keywords" um welche hinzuzufügen'
								: 'Keine Keywords verfügbar'}
						</div>
					{/if}
					<div class="geo-navigation-inline">
									...
									<div class="geo-edit-actions">
										{#if image?.lat && image?.lon}
											<button
												type="button"
												class="geo-edit-btn"
												on:click={autofillGeoFieldsFromCoordinates}
												>Ortsdaten aus GPS ermitteln</button
											>
										{/if}
										<button
											type="button"
											class="geo-edit-btn geo-edit-btn--primary"
											on:click={saveGeoFields}>Speichern</button
										>
										<button type="button" class="geo-edit-btn" on:click={cancelEditGeoFields}
											>Abbrechen</button
										>
									</div>
								{:else}
									<button
										type="button"
										class="geo-edit-btn"
										class:geo-edit-btn--warn={geoNeedsAttention}
										on:click={startEditGeoFields}
									>
						...
						<nav class="geo-breadcrumbs" aria-label="Geo-Breadcrumb">
							{#each geoDisplayBreadcrumbs as crumb, index}
								{#if crumb.path}
									<a
										href={crumb.path}
										aria-current={index === geoDisplayBreadcrumbs.length - 1 ? 'page' : undefined}
										>{crumb.name}</a
									>
								{:else}
						...
						<div class="geo-field-grid">
										...
										<div class="geo-search-results">
											{#each geoSearchResults as result}
												<button
													...
													on:click={() => selectGeoSearchResult(result)}
												>
							...
							<div class="geo-field">
								...
								{#if editingGeoFields}
									<input
										bind:value={countryNameEditValue}
										class="geo-input"
							...
							<div class="geo-field">
								...
								{#if editingGeoFields}
									<input
										bind:value={municipalityNameEditValue}
										class="geo-input"
										placeholder="Reischach"
									/>
								{:else}
					...
					{#if isCreator}
						<div class="adobe-stock-card">
							...
							{#if image?.adobe_stock_uploaded_at}
								<div class="meta-line">
									Upload: {new Date(image.adobe_stock_uploaded_at).toLocaleString('de-DE')}
								</div>
							{/if}
							...
							<div class="adobe-actions">
								...
								<button
									class="adobe-btn primary"
									on:click={uploadToAdobeStock}
									disabled={adobeUploadLoading}
								>
							...
							{#if image?.adobe_stock_url}
								<a
									...
									href={getAdobeAffiliateUrl(image.adobe_stock_url) || image.adobe_stock_url}
									target="_blank"
									rel="noopener noreferrer sponsored">Adobe Link öffnen</a
								>
							{/if}
						</div>
					{/if}
				...
				<div class="column-card">
					...
					{#if image.profile}
						<div class="creator-header">
							{#if image.profile.avatar_url}
								<button
									...
									title={`Nur Bilder von ${attribution?.creatorName ?? image.profile.full_name} anzeigen`}
								>
									<img
										src={image.profile.avatar_url.startsWith('http')
											? image.profile.avatar_url
											: `https://caskhmcbvtevdwsolvwk.supabase.co/storage/v1/object/public/avatars/${image.profile.avatar_url}`}
										alt="Avatar"
										class="avatar"
									/>
								</button>
						...
						<div class="creator-details">
							<button
								type="button"
								class="creator-name clickable-name"
								on:click={setUserFilter}
								title={`Nur Bilder von ${attribution?.creatorName ?? image.profile.full_name} anzeigen`}
							>
								{attribution?.creatorName ?? image.profile.full_name}
							</button>
							...
							<div class="creator-contact">
								...
								{#if image.profile.show_website && image.profile.website}
									<div>
										...
										<a href={image.profile.website} target="_blank" rel="noopener noreferrer"
											>{image.profile.website}</a
										>
									</div>
								{/if}
							</div>
							<div class="creator-socials">
								...
								{#if image.profile_id}
									<FollowButton
										...
										targetLabel={attribution?.creatorName ||
											image.profile.full_name ||
											image.profile.accountname ||
											'Profil'}
										iconOnly={true}
									/>
								{/if}
			...
			<ImageMapSection
				...
				{isCreator}
				hasEditRights={$unifiedRightsStore.rights?.edit}
				on:openMapPicker={() => {
					showMapPicker = true;
				}}
			/>
