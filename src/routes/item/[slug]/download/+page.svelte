<script lang="ts">
	...
	$: isCreator =
		!!currentUser && (currentUser.id === image?.profile_id || currentUser.id === image?.user_id);
	$: canDownload = !!(
		isCreator ||
	...
	$: if (
		...
		canDownload &&
		originalEstimateKey &&
		!originalEstimateLoaded &&
		!originalEstimateLoading
	) {
		fetchOriginalEstimate();
	}

	$: downloadAttributionForKey = attribution;
	$: metadataPreviewKey = JSON.stringify({
		...
		creator: downloadAttributionForKey?.creatorName ?? null,
		copyright: downloadAttributionForKey?.copyrightNotice ?? null,
	...
	function findNestedMetadataText(
		source: unknown,
		matchers: Array<(key: string) => boolean>
	): string | null {
	...
	function getCulocaMetadataPreview(): MetadataPreview {
		...
		const creator = attribution?.creatorName ?? null;
		const copyright = attribution?.copyrightNotice ?? null;
		...
		return {
			...
			keywords: firstText(
				Array.isArray(image?.keywords) ? image.keywords.join(', ') : image?.keywords
			),
			creator,
			copyright,
			gps:
	...
	function buildRequestOptions() {
		return {
			...
			format: settings.format,
			compression:
				settings.quality == null ? null : Math.max(1, Math.min(99, 100 - settings.quality)),
	...
	function sanitizeExportSettings(value: unknown): ExportSettings {
		...
		return {
			...
			format,
			quality: parsed.quality == null ? null : Math.max(25, Math.min(85, Number(parsed.quality))),
			metadataMode,
	...
	async function loadDownloadPresets(userId: string) {
		...
		try {
			...
			const presets = Array.isArray(data?.download_presets)
				? (data.download_presets
						.map((entry: unknown) => sanitizePreset(entry))
						.filter(Boolean) as DownloadPreset[])
				: [];
	...
	async function handleSavePreset() {
		...
		const dedupedPresets = downloadPresets.filter(
	...
	function handlePointerMove(event: PointerEvent) {
		...
		if (dragState.type === 'resize-se') {
			maxWidth = Math.min(
				1 - dragState.startRect.x,
				(1 - dragState.startRect.y) * maxWidthByHeight
			);
	...
	function getStorageModeLabel() {
		...
		const nameLabel =
			settings.filenameMode === 'original' ? 'Originaldateiname' : 'Culoca-Dateiname';
...
<main class="download-page">
	<div class="download-shell">
		<div class="download-header">
			...
			<h1>Download</h1>
			<p>
				Volle Auflösung und Originaldateiname sind vorausgewählt. Bei Bedarf passen wir Größe,
				Format, Zuschnitt und Metadaten vor dem Export an.
			</p>
		</div>
		...
		<div class="download-grid">
			<section class="preview-card">
				<div
					...
					style={`aspect-ratio:${getPreviewAspectRatio()};`}
				>
					{#if previewImageUrl}
						<img
							src={previewImageUrl}
					...
					{#if settings.sizeMode === 'custom' && settings.cropEnabled}
						...
						>
							...
							<button
								...
								on:pointerdown|stopPropagation={(event) => startCropDrag('resize-nw', event)}
								aria-label="Zuschnitt oben links anpassen"
							></button>
							<button
								type="button"
								class="crop-handle ne"
								on:pointerdown|stopPropagation={(event) => startCropDrag('resize-ne', event)}
								aria-label="Zuschnitt oben rechts anpassen"
							></button>
							<button
								type="button"
								class="crop-handle sw"
								on:pointerdown|stopPropagation={(event) => startCropDrag('resize-sw', event)}
								aria-label="Zuschnitt unten links anpassen"
							></button>
							<button
								type="button"
								class="crop-handle se"
								on:pointerdown|stopPropagation={(event) => startCropDrag('resize-se', event)}
								aria-label="Zuschnitt unten rechts anpassen"
							></button>
						</div>
					{/if}
				</div>
			...
			<section class="settings-card">
				<div class="group">
					...
					<div class="radio-row">
						<label
							><input type="radio" bind:group={settings.sizeMode} value="full" /> Volle Auflösung</label
						>
						<label
							><input type="radio" bind:group={settings.sizeMode} value="custom" /> Eigene Auflösung</label
						>
					</div>
					...
					{#if settings.sizeMode === 'custom'}
						<div class="dimension-grid">
							...
							<label>
								...
								<input
									...
									on:input={() => syncDimensions('height')}
								/>
							</label>
						</div>
						...
						<label class="checkbox">
							<input
								type="checkbox"
								bind:checked={settings.cropEnabled}
								on:change={handleCropToggle}
							/>
				...
				{#if settings.format === 'jpg'}
					<div class="group">
						...
						<div class="radio-stack">
							<label
								><input type="radio" bind:group={settings.metadataMode} value="original" /> Original
								EXIF / ICMP / GPS</label
							>
							<label
								><input type="radio" bind:group={settings.metadataMode} value="culoca" /> Culoca EXIF
								/ ICMP / GPS</label
							>
							<label
								><input type="radio" bind:group={settings.metadataMode} value="none" /> Ohne EXIF / ICMP
								/ GPS</label
							>
						</div>
					</div>
				...
				<div class="group">
					...
					<div class="radio-stack">
						<label
							><input
								type="radio"
								checked={settings.filenameMode === 'original'}
								on:change={() => setFilenameMode('original')}
							/> Originaldateiname</label
						>
						<label
							><input
								type="radio"
								checked={settings.filenameMode === 'web'}
								on:change={() => setFilenameMode('web')}
							/>
							Webtaugliche Dateibenennung + culoca-{image?.short_id ||
								image?.id?.slice(0, 10)}</label
						>
					</div>
				</div>
				...
				<div class="actions">
					...
					<button
						...
						disabled={isDownloading ||
							(!!currentUser && !canDownload && !$unifiedRightsStore.loading)}
					>
						{#if !currentUser}
		...
		<section class="metadata-section">
				...
				{#if settings.format !== 'jpg'}
					<div class="metadata-line">
						...
						<span class="metadata-value"
							>Bei WebP werden aktuell keine EXIF-, IPTC- oder GPS-Daten eingebettet.</span
						>
					</div>
				{:else}
					<div class="metadata-line">
			...
			{#if settings.format === 'jpg'}
				...
				{:else}
					<p class="metadata-note">
						Ohne EXIF / IPTC / GPS entfernt alle eingebetteten Metadaten aus der Exportdatei.
					</p>
				{/if}
			{/if}
		</section>
...
<style>
	...
	.preview-media {
		...
		background: linear-gradient(
			180deg,
			var(--download-preview-top),
			var(--download-preview-bottom)
		);
		aspect-ratio: 4 / 3;
		user-select: none;
	}
