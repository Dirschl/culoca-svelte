  ...
  try {
    ...
    while (hasMore) {
      const { data: items, error } = await supabase
        .from('items')
        .select('id, title, description, caption, slug, lat, lon, path_64, path_512, created_at, user_id')
        .not('lat', 'is', null)
        .not('lon', 'is', null)
        .eq('is_private', false)
        .eq('gallery', true)
        ...
        <BalloonStyle>
          ...
          <text><![CDATA[
            <div style="font-family: Arial, sans-serif; max-width: 500px;">
              ...
              <p style="font-size: 0.8em; color: #666;"><strong>👤 Erstellt von:</strong> ${item.user_id || 'Unbekannt'}</p>
              ...
              ${item.path_512 ? `<p style="margin-top: 15px;"><img src="https://caskhmcbvtevdwsolvwk.supabase.co/storage/v1/object/public/images-512/${item.path_512}" style="width: 200px; height: auto; border: 3px solid #ff6600; border-radius: 8px;" alt="${title}" /></p>` : ''}
            </div>
          ]]></text>
        </BalloonStyle>
      </Style>
