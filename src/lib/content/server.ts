...
async function getSemanticSimilarItems(
	...
	try {
		...
		const fallbackMatches =
			localMatches.length >= PREFERRED_SIMILARITY_COUNT ? [] : await fetchMatches(null);
		...
		const ids = dedupedMatches.map((row: SimilarityMatchRow) => row.item_id).filter(Boolean);
		...
		return orderedRows.map((row) => ({
			...row,
			similarity:
				dedupedMatches.find((match: SimilarityMatchRow) => match.item_id === row.id)?.similarity ||
				0
		}));
	} catch {
		return [];
...
function formatSlugToTitle(slug: string | null | undefined): string | null {
	if (!slug) return null;
...
function resolveCountryNameForPayload(item: ItemRecord): string {
	const trimmedDb = item.country_name && String(item.country_name).trim();
	if (trimmedDb) return trimmedDb;
	const enriched = enrichItemCountryName(item);
	if (enriched && enriched.trim()) return enriched.trim();
	return (
		formatSlugToTitle(item.country_slug) ||
...
export async function loadContentPage(args: {
	...
	const canonicalPath = getStoredOrComputedCanonicalPath({ item, rootItem, type });
	if (
		!args.skipCanonicalRedirect &&
		canonicalPath &&
		normalizePath(canonicalPath) !== requestedPath
	) {
		return {
	...
	const siblings = [rootItem, ...groupItems]
		.filter(
			(candidate, index, list) => list.findIndex((other) => other.id === candidate.id) === index
		)
	...
	const availableTypes = Array.from(typeMap.values())
		.filter(
			(candidate): candidate is Partial<ContentTypeDefinition> & { id: number } =>
	...
	return {
		...
		seoHubs: {
			...
			photographerPath: profileData.profile?.accountname
				? buildPhotographerHubPath(profileData.profile.accountname as string)
				: null,
