 ...
 * noimageindex, damit Google Bilder ein klares Hauptmotiv pro Item-URL sieht.
 */
...
export async function respondPublicImage(
	...
	try {
		...
		const { data: rows, error: dbError } = await dbForPublicImages
			.from('items')
			.select(
				'id, slug, path_2048, path_512, title, description, is_private, created_at, updated_at'
			)
			.eq('slug', actualSlug)
			.limit(1);
		...
		if (!hit) {
			console.error(
				`❌ [Images] No object found in storage for item ${item.id} (slug ${actualSlug})`
			);
			return new Response('Image not found', {
				status: 404,
				headers: {
