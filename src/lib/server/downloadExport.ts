...
export type DownloadableItem = {
	...
	profile?: {
		// Für „Culoca“ brauchen wir die neuen Attribution-Felder.
		// (nullable, weil ältere DB-Einträge evtl. noch keine Migration ausgefüllt haben)
...
const DEFAULT_OPTIONS: Required<
	Pick<DownloadExportOptions, 'sizeMode' | 'format' | 'metadataMode' | 'filenameMode'>
> = {
	sizeMode: 'full',
	format: 'jpg',
...
export function normalizeDownloadExportOptions(input: unknown): DownloadExportOptions {
	...
	const metadataMode =
		source.metadataMode === 'none' ||
		source.metadataMode === 'culoca' ||
		source.metadataMode === 'original'
			? source.metadataMode
	...
	const compression =
		compressionRaw == null || compressionRaw === ''
			? null
			: clampNumber(compressionRaw, 15, 75, 15);
	const width =
		source.width == null || source.width === '' ? null : clampNumber(source.width, 32, 12000, 2048);
	const height =
		source.height == null || source.height === ''
			? null
			: clampNumber(source.height, 32, 12000, 2048);
	const cropSource = (
		source.crop && typeof source.crop === 'object' ? source.crop : null
...
export async function fetchOriginalItemBuffer(
	...
	const loader = (async () => {
		...
		if (originalUrl) {
			...
			if (process.env.HETZNER_WEBDAV_USER && process.env.HETZNER_WEBDAV_PASSWORD) {
				try {
					...
					console.warn(
						'Authenticated original fetch failed, falling back to WebDAV:',
						authFetchError
					);
				}
			}
		...
		if (
			...
			!process.env.HETZNER_WEBDAV_USER ||
			!process.env.HETZNER_WEBDAV_PASSWORD
		) {
		...
		try {
			...
			loadedBuffer = Buffer.isBuffer(fileBuffer)
				? fileBuffer
...
function buildOriginalExif(item: DownloadableItem, width: number, height: number) {
	...
	return {
		...
		IFD3: stringifyExifValues(
			buildGpsInfo(
				exif.latitude ?? exif.Latitude ?? item.lat,
				exif.longitude ?? exif.Longitude ?? item.lon
			)
		)
	};
}
...
function buildCulocaExif(item: DownloadableItem, width: number, height: number) {
	...
	const attribution = resolveAttribution({
		...
		profile: item.profile as AttributionProfileSettings,
		mode: 'culoca'
	});
	const artist = attribution.creatorName;
	const copyright = attribution.copyrightNotice;
	...
	const description = firstString(
		item.description,
		original.IFD0.ImageDescription,
		caption,
		'Culoca Export'
	);
...
function buildCulocaXmp(item: DownloadableItem) {
	...
	const creator = attribution.creatorName;
	...
	const copyright = attribution.copyrightNotice;
	const usageTerms = attribution.licenseUrl;
...
export function canRewriteMetadataWithoutSharp(
...
function canReturnOriginalBufferUnchanged(metadata: Metadata, options: DownloadExportOptions) {
	return (
...
export function canBypassImageProcessing(options: DownloadExportOptions) {
	return (
		options.sizeMode === 'full' && options.format === 'jpg' && options.metadataMode === 'original'
	);
}
...
export async function renderDownloadExport(
	...
	if (options.metadataMode === 'original' && options.format === 'jpg') {
		...
		try {
			...
			result = await renderTransformedBuffer(
				rebuildProcessedPipeline().withMetadata({
					exif: culocaExif
				})
			);
		}
