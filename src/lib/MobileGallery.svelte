  ...
  async function loadInitialGridImages(lat: number, lon: number) {
    ...
    const converted = newImages.map((img: any) => {
      ...
      return {
        ...
        country_slug: img.country_slug ?? null,
        state_slug: img.state_slug ?? null,
        region_slug: img.region_slug ?? null,
        district_slug: img.district_slug ?? null,
        municipality_slug: img.municipality_slug ?? null,
