-- Fix User Filter SQL Logic
-- Check and recreate the functions with correct user filter logic

-- First, let's check if the functions exist and their current logic
SELECT 
  proname as function_name,
  proargnames as parameter_names,
  proargtypes::regtype[] as parameter_types
FROM pg_proc 
WHERE proname IN ('gallery_items_normal_postgis', 'gallery_items_search_postgis')
ORDER BY proname;

-- Drop all versions to avoid conflicts
DROP FUNCTION IF EXISTS public.gallery_items_normal_postgis(double precision, double precision, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.gallery_items_normal_postgis(double precision, double precision, integer, integer, uuid, text);
DROP FUNCTION IF EXISTS public.gallery_items_search_postgis(double precision, double precision, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.gallery_items_search_postgis(double precision, double precision, integer, integer, uuid, text);

-- Create the normal gallery function with correct user filter logic
...
-- Create the search function with correct user filter logic
...
-- Test the functions
SELECT 'Testing normal function with user filter' as test;
SELECT COUNT(*) as normal_with_user_filter FROM gallery_items_normal_postgis(0, 0, 0, 50, 'aae40790-a31e-4c21-a03b-762f513f52af'::uuid);

SELECT 'Testing normal function without user filter' as test;
SELECT COUNT(*) as normal_without_user_filter FROM gallery_items_normal_postgis(0, 0, 0, 50, NULL);

SELECT 'Testing search function with user filter' as test;
SELECT COUNT(*) as search_with_user_filter FROM gallery_items_search_postgis(0, 0, 0, 50, 'aae40790-a31e-4c21-a03b-762f513f52af'::uuid, 'wasser');

SELECT 'Testing search function without user filter' as test;
SELECT COUNT(*) as search_without_user_filter FROM gallery_items_search_postgis(0, 0, 0, 50, NULL, 'wasser'); 