<script lang="ts">
  ...
  async function loadInitialGridImages(lat: number, lon: number) {
    ...
    console.log(`[Gallery Grid] Set dynamic loader user ID: ${currentUserId} for grid loading`);
    ...
    pics.set(converted);
      ...
      const totalCount = await getTotalImageCount();
      console.log(`[Gallery Grid] Total images in database: ${totalCount}, loaded: ${converted.length}`);
      
      if (converted.length < totalCount) {
        console.log(`[Gallery Grid] More images available, will continue loading`);
        hasMoreImages = true;
      } else {
        console.log(`[Gallery Grid] All images loaded`);
        hasMoreImages = false;
      }
  }
  ...
  async function loadAllImagesFromDatabase() {
    ...
    try {
      ...
      if (hasUserFilter && currentFilters.userFilter) {
        ...
        query = query.or(`profile_id.eq.${currentUserId},is_private.eq.false,is_private.is.null`);
        console.log(`[Gallery Database] Logged in user filter applied`);
      } else {
        query = query.or('is_private.eq.false,is_private.is.null');
  ...
  async function getTotalImageCount() {
    try {
      ...
      if (isLoggedIn && currentUser) {
        ...
        console.log(`[Gallery Total Count] Logged in user filter applied`);
      } else {
  ...
  if (page === 0 && reason.includes('initial')) {
    console.log(`[Gallery] Initial load detected, using intelligent loader system`);
  ...
  if (reason.includes('GPS failed')) {
    console.log(`[Gallery] GPS failed fallback detected, loading all images from database...`);
